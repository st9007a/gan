...
d_loss = -tf.reduce_mean(d_real + 1. - d_fake)
g_loss = -tf.reduce_mean(d_fake)

d_clip = [val.assign(tf.clip_by_value(val, -0.01, 0.01)) for val in var_d]

d_opt = tf.train.RMSPropOptimizer(0.0005).minimize(d_loss, var_list = var_d)
g_opt = tf.train.RMSPropOptimizer(0.0005).minimize(g_loss, var_list = var_g)
...
for step in range(1, 5001):
    for k in range(25):
        ...
        _, d_loss_train, d_clip_train = sess.run([d_opt, d_loss, d_clip], feed_dict = {x_d: batch_x, x_g: sample_z(batch_size, g_dim)})
